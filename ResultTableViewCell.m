...
- (void)setupUI{
    self.infoContainer = [[UIView alloc]initWithFrame:CGRectMake(APPLICATION_FRAME.size.width/2 - APPLICATION_FRAME.size.width * 0.465, RESULT_CELL_HEIGHT * 0.03, APPLICATION_FRAME.size.width * 0.93, RESULT_CELL_HEIGHT * 0.96)];
    self.infoContainer.backgroundColor = UIColorFromRGB(0xF9F9F9);
    self.infoContainer.layer.cornerRadius = 7.0;
    self.infoContainer.layer.borderColor = UIColorFromRGB(0x979797).CGColor;
    self.infoContainer.layer.borderWidth = 1.0;
    [self.contentView addSubview:self.infoContainer];
    
    self.restaurantName = [[UILabel alloc]initWithFrame:CGRectMake(self.infoContainer.bounds.size.width * 0.03, 7.0, self.infoContainer.bounds.size.width * 0.9, self.infoContainer.bounds.size.height * 0.15)];
...
//    [self.infoContainer addSubview:self.sepLine];
    ...
    self.ratingLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMinX(self.restaurantName.frame), CGRectGetMaxY(self.categoryLabel.frame) + self.infoContainer.frame.size.height * 0.03, self.infoContainer.frame.size.width * 0.2, self.infoContainer.frame.size.height * 0.18)];
    self.ratingLabel.backgroundColor = [UIColor clearColor];
    self.ratingLabel.textColor = APPLICATION_BLUE_COLOR;
    self.ratingLabel.font = [UIFont nun_fontWithSize:self.infoContainer.frame.size.height * 0.16];
    [self.infoContainer addSubview:self.ratingLabel];
    
    self.openIcon = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.ratingLabel.frame) + self.infoContainer.frame.size.width * 0.03, CGRectGetMinY(self.ratingLabel.frame) + self.infoContainer.frame.size.height * 0.03, self.infoContainer.bounds.size.width * 0.04, self.infoContainer.bounds.size.width * 0.04)];
...
//    [self.infoContainer addSubview:self.overallRating];
    ...
    self.priceSign = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMinX(self.openIcon.frame), CGRectGetMaxY(self.openIcon.frame) + self.infoContainer.bounds.size.height * 0.07, self.openIcon.frame.size.width, self.openIcon.frame.size.height)];
...
- (void)populateRestaurant:(TPLRestaurant *)restaurant{
    ...
    if (restaurant.openNowExplore != nil) {
        ...
        self.openNowLabel.text = @"Unavailable";
    }
    ...
    if (restaurant.foursq_rating) {
        NSNumber *roundedRating = @(restaurant.foursq_rating.doubleValue * 10.0);
        self.ratingLabel.text = [NSString stringWithFormat:@"%ld%%", roundedRating.integerValue];
    }
